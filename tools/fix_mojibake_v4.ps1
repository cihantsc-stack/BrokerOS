...
$backup = Join-Path $root ".croc_backups\mojibake_v4_$stamp"
...
function Repair-Line([string]$line) {
  if ($line -notmatch $markerPattern) { return $line }

  try {
    $bytes = $cp1252.GetBytes($line)
    $candidate = [System.Text.Encoding]::UTF8.GetString($bytes)

    $before = ([regex]::Matches($line, $markerPattern)).Count
    $after = ([regex]::Matches($candidate, $markerPattern)).Count

    if ($after -lt $before -and $candidate.IndexOf([char]0xFFFD) -lt 0) {
      return $candidate
    }
  } catch {}

...
class MemoryPredictionRepository implements PredictionRepository {
  ...
  @override
  List<CrocPrediction> predictionsFor(String symbol)=>List<CrocPrediction>.unmodifiable(_records[symbol.toUpperCase()] ?? const <CrocPrediction>[]);
  ...
  void saveAll(List<CrocPrediction> predictions){
    ...
    for(final p in predictions){
      final exists=bucket.any((x)=>x.horizon==p.horizon && x.snapshot.createdAt==p.snapshot.createdAt);

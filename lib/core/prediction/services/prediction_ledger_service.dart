...
class PredictionLedgerService {
  ...
  static final PredictionLedgerService instance =
      PredictionLedgerService._();

  static const String _storageKey =
      'croc_prediction_ledger_v3';

  Future<void> captureAndReconcile(
    StockAnalysis stock,
  ) async {
    ...
    if (!alreadyCapturedToday) {
      ...
      for (final prediction in predictions) {
        predictionMap[prediction.horizon.name] =
            <String, dynamic>{
          'modelProbability':
      ...
      records.insert(
        ...
        <String, dynamic>{
          'id':
              '${symbol}_${now.microsecondsSinceEpoch}',
          'symbol': symbol,
  ...
  Future<Map<String, dynamic>> accuracyFor(
    String symbol,
  ) async {
    ...
    for (final horizon
      ...
      result[horizon.name] =
        ...
        'successRate': resolved == 0
            ? 0.0
            : successful / resolved * 100,
        'averageReturnPct': resolved == 0
            ? 0.0
            : totalReturn / resolved,
      };
    }
  ...
  void _reconcile({
    ...
    for (final record in records) {
      ...
      for (final horizon
        ...
        final maturedAt = _addTradingDays(
          createdAt,
          horizon.tradingDays,
        );
